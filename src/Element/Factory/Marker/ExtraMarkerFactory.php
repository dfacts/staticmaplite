...
namespace StaticMapLite\Element\Factory\Marker;

use StaticMapLite\Element\Marker\ExtraMarker;
use StaticMapLite\Exception\ExtraMarker\InvalidColorException;
use StaticMapLite\Exception\ExtraMarker\InvalidIconException;
use StaticMapLite\Exception\ExtraMarker\InvalidShapeException;

class ExtraMarkerFactory
{
    public function create(float $latitude, float $longitude, string $shape, string $color, string $icon): ExtraMarker
    {
        return new ExtraMarker($this->getShape($shape), $this->getColor($color), $this->getAwesome($icon), $latitude, $longitude);
    }

    protected function getAwesome(string $icon): string
    {
        $icons = [
            'glass'                               => 'f000',
            'music'                               => 'f001',
        ...
        if (!array_key_exists($icon, $icons)) {
            throw new InvalidIconException(sprintf('The icon "%s" is not available', $icon));
        }

        return $icons[$icon];
    }

    protected function getShape(string $shape): int
    {
        $shapes = ['circle', 'square', 'star', 'penta'];

        $key = array_search($shape, $shapes);

        if (false === $key) {
            throw new InvalidShapeException(sprintf('The shape "%s" is not available', $shape));
        }
    ...
    protected function getColor(string $color): int
    {
        ...
        if (false === $key) {
            new InvalidColorException(sprintf('The color "%s" is not available', $color));
        }
